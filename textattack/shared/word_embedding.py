...
class WordEmbedding(AbstractWordEmbedding):
    ...
    def counterfitted_GLOVE_embedding():
        ...
        word_embeddings_file = "/home/guest/r09944010/2020MLSECURITY/final/ml-security-proj/paragram_words.npy"
        word_list_file = '/home/guest/r09944010/2020MLSECURITY/final/ml-security-proj/wordlist_words.npy'
        ...
        for word in embedding_matrix:
            ...
            new_embedding_matrix.append(temp)

        word2index = np.load(word_list_file, allow_pickle=True).item()
        index2word = {}
        for word, index in word2index.items():
            index2word[index] = word
        # nn_matrix = np.load(nn_matrix_file)

        embedding = WordEmbedding(new_embedding_matrix, word2index, index2word)
...
class BertEmbedding(AbstractWordEmbedding):
    """Object for loading word embeddings and related distances for TextAttack.
    This class has a lot of internal components (e.g. get consine similarity)
    implemented. Consider using this class if you can provide the appropriate
    input data to create the object.
    ...
    def __init__(self,nn_matrix=None):
        self.embedding_matrix =  AutoModel.from_pretrained('bert-chinese-case')
