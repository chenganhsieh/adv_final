...
class WordSwapMaskedLM(WordSwap):
    ...
    def __init__(
        self,
        method="bae",
        masked_language_model="bert-base-chinese",
        tokenizer=None,
        max_length=30,
        max_candidates=20,
        min_confidence=5e-4,
    ...
    def _encode_text(self, text):
        ...
        a = {k: v.to(utils.device) for k, v in encoding.items()}
            ...
            for j in range(len(ids)):
                ...
                for _id in ranked_indices:
                    ...
                    if (
                        ...
                        and utils.is_one_word(token)
                        and not utils.check_if_punctuations(token)
                        ):
                            top_words.append(token)
                    # print(token)
                    # print(self._language_model.config.model_type)
                    # word_temp = utils.strip_BPE_artifacts(
                    #         token, self._language_model.config.model_type
                    # )
                    # print( mask_token_probs[_id])
